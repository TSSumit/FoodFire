...
* In Diff algorithm, current tree is compared with the new tree and the different is reflected on DOM.
* React Fibre is the updated reconciliation algo

  ![1706013338035](image/theory/1706013338035.png)
* React is fast because of its fast DOM manipulation.
* Diff algo 'detect' what exactly got changed in the page and it'll just change that while re-rendering the whole tree.

## Q: What is React useState Hook?

* The useState hook is a React Hook that allows functional components to manage state in a functional way.
* It is part of the Hooks API introduced in React 16.8.
...
The setState function can be asynchronous, so React batches state updates for better performance.
It's recommended to use functional updates when the new state depends on the previous state to avoid race conditions.
Multiple state variables can be declared using multiple useState hooks.
useState can also accept a function as the initial state, which is useful for expensive calculations.
...
![1706120603692](image/theory/1706120603692.png)
